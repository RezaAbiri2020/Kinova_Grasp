...
while np.abs(delta_x) > 0.0001 or np.abs(delta_y) > 0.0001 or np.abs(delta_z) > 0.0001 :
  ...
  if delta > inputRate:
    ...
    eulOrn = p.getEulerFromQuaternion(orn)
    #print(eulOrn)
    # desgin of a PD controller for set-point control based on force control
    # Control law performs a nonlinear compensating action of joint space gravitational forces 
    # and an operational space linear PD control action.
    newPosInput = 1
    ...
    numJoints = p.getNumJoints(jacoId)
    #PosJoints, VelJoints, TorqJoints = p.getJointStates(jacoId,jointIndices = [2,3,4])
    JS2, JS3, JS4, JS5, JS6, JS7, JS9, JS11, JS13 = p.getJointStates(jacoId, [2, 3, 4, 5, 6, 7, 9, 11, 13])
    
    PosJoints = [JS2[0], JS3[0], JS4[0], JS5[0], JS6[0], JS7[0], JS9[0], JS11[0], JS13[0]]
    result = p.getLinkState(jacoId, jacoEndEffectorIndex, computeLinkVelocity=1, computeForwardKinematics=1)
    link_trn, link_rot, com_trn, com_rot, frame_pos, frame_rot, link_vt, link_vr = result
   
    zero_vec = [0.0] * 9
    jac_t, jac_r = p.calculateJacobian(jacoId, jacoEndEffectorIndex, com_trn, PosJoints, zero_vec, zero_vec)
